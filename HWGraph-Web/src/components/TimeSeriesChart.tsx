...
import { exportChartAsPNG, exportChartAsSVG, exportToCSV } from '../utils/chartExport';
...
export function TimeSeriesChart({ files, selectedSensors }: TimeSeriesChartProps) {
  ...
  const handleExportPNG = () => {
    if (chartInstanceRef.current) {
      exportChartAsPNG(chartInstanceRef.current, `hwgraph-${Date.now()}.png`);
  ...
  const handleExportSVG = () => {
    if (chartInstanceRef.current) {
      exportChartAsSVG(chartInstanceRef.current, `hwgraph-${Date.now()}.svg`);
      setShowExportMenu(false);
    }
  };
  ...
  const handleExportCSV = () => {
    if (chartInstanceRef.current) {
      ...
      exportToCSV((option as any).series, `hwgraph-${Date.now()}.csv`);
      setShowExportMenu(false);
    }
  };
        ...
        {selectedSensors.size > 0 && (
          <div className="flex gap-2">
            ...
            <div className="relative">
              ...
              {showExportMenu && (
                <div className="absolute right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-50 min-w-[150px]">
                  <button onClick={handleExportPNG} className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-gray-700 rounded-t-lg">
                    Export as PNG
                  </button>
                  <button onClick={handleExportSVG} className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-gray-700">
                    Export as SVG
                  </button>
                  <button onClick={handleExportCSV} className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-gray-700 border-t border-gray-700 rounded-b-lg">
                    Export as CSV
                  </button>
                </div>
              )}
