...
const App = () => {
  ...
  // Helper to extract filename from File object
  ...
  useEffect(() => {
    ...
    // Prevent file from being opened in browser when dropped outside drop zone
  ...
  const handleDrop = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (file.name.toLowerCase().endsWith('.csv')) {
        ...
        setError('Please drop a CSV file (received: ' + file.name + ')');
      }
    }
  };
  ...
  const handleLoadFile = async () => {
    ...
    try {
      ...
      console.log('File read successfully, size:', selectedFile.size);
      console.log('Parsing CSV...');

      // Parse CSV (will use Web Worker in next step)
